      ...
      try {
        ...
        if (allPosts.data.posts.length) {
          ...
          if (id) {
            ...
            setTitleArr(await allPosts.data.posts[index]._doc.title.split(" "));
            dispatch(postActions.setSelPost(allPosts.data.posts[index]));
          }
        }
        setLoading(false);
      } catch (error) {
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center mr-2 justify-center rounded-full">
                    <FaComment size={30} className="m-2 text-[#b4b4b4]" /> 0
                  </div>
                </motion.div>
              </motion.div>
                ...
                <div id="posts">
                  ...
                  {postData.length > 0 &&
                    ...
                    postData.map((posts, index) => {
                      return (
                        selPost._doc.title !== posts._doc.title && (
                          <Link
                            to={`/${posts._doc._id}`}
                            onClick={test}
                            key={index}
